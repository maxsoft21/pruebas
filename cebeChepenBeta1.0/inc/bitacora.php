<section id="cd-timeline" class="cd-container">
    <?php
        $selectBita=ejecutarSQL::consultar("select * from asistencia a inner join usuario u 
        on a.idUsuario=u.idUsuario inner join rol r 
        on r.idRol=u.idRol ORDER BY STR_TO_DATE(a.Fecha,'%d-%m-%Y') DESC, a.Entrada DESC LIMIT 15");
        $c=0;
        while($row=mysqli_fetch_array($selectBita)){
            $c=$c+1;
            if($row['idRol']=='1'){
                $selectName=ejecutarSQL::consultar("SELECT * FROM usuario WHERE idUsuario='".$row['idUsuario']."'");
