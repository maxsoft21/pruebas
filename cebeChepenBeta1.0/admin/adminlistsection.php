    ...
    <div class="content-page-container full-reset custom-scroll-containers">
        ...
        <div class="container">
            <div class="page-header">
              <h1 class="all-tittles">Sistema cebe chepén <small>- Administración Institución</small></h1>
            </div>
        </div>
        ...
        <div class="container-fluid">
            ...
            <?php 
                $checkSection= ejecutarSQL::consultar("SELECT * FROM seccion where CodigoSeccion!=1 ORDER BY Nombre");
                $checktotal=mysqli_num_rows($checkSection);
                if($checktotal>0){
                    $p=1;
                    while($fila=mysqli_fetch_array($checkSection)){
                        $checkSectionTeacher= ejecutarSQL::consultar("SELECT * FROM docente d inner join
                        datospersonales dp on dp.dni= d.dni inner join
                        seccion s on s.CodigoSeccion=dp.CodigoSeccion WHERE dp.CodigoSeccion='".$fila['CodigoSeccion']."'");
                        $checkSectionStudent= ejecutarSQL::consultar("SELECT * FROM alumno a inner join
                        datospersonales dp on dp.dni= a.dni inner join
                        seccion s on s.CodigoSeccion=dp.CodigoSeccion WHERE dp.CodigoSeccion='".$fila['CodigoSeccion']."'");
